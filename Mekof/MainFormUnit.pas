...
type
  TReferenceTerm = class(TObject)
  public
    Constructor Create(Text: string; Rule: TReferenceDef);
  end;
...
Constructor TReferenceTerm.Create(Tag: string; Length: string;
...
begin
  ...
  Temp := copy(SourceText, 25, i);
end;

function TMekofRecord.ParseMarker(Marker: string): TMekofMarker;
begin
...
function TMekofRecord.ParseReference(Reference: string): TMekofFieldList;
var
  ...
  Temp: string;
begin
  self.ReferenceDef := TReferenceDef.Create();
  ...
  TermCount := (ReferenceLen div TermLength) + 1;
  SetLength(self.Fields, TermCount);
  for i := 0 to TermCount - 1 do
    begin
      Temp := copy(Reference, 24 * TermCount + 1, TermLength);
      self.Fields[i] :=
    end;
end;
{$ENDREGION}
